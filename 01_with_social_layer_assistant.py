...
def main():
    ...
    # after first constraint, we can get following user set
    user_set_after_success_rate, group_list_after_success_rate = success_rate_limit(user_poisson, user_set, num_of_requests, ksi) # 3 users, 0.5-0.6 filter
    print 'After success rate limit, remove unqualified request combination and user, the group list is '
    print group_list_after_success_rate
    print '**********************'
    # second constraint

    # get random position of points recommended from social layer(device_position_list_target) and all points
    device_position_list_target, device_position_list_all = get_random_position(radius, num_of_points, user_set_after_success_rate)  # format is (id, (x,y))

    # use transmission rate to remove bad group

    # first three points in device_position_list_target is based, celluar, request user
    device_position_list_target = [device_position_list_target[0], device_position_list_all[-1], device_position_list_all[-2]] + device_position_list_target[1:]
    group_final = group_after_transmission_rate_limit(p_s_c, p_t_d, gamma_g, gamma_h, sigma_2, epsilon, device_position_list_target, group_list_after_success_rate, unit_func)
    print 'After transmission limit, remove unqualified request combination and user, the group list is '
    print group_final
    print '**********************'

    transmission_rank = transmission_rank_result(p_j_d, p_i_c, sigma_2, gamma_g, gamma_h, unit_func, user_poisson, group_final, device_position_list_target)
    print 'Calculate T_d for all request combinations'
    print transmission_rank
